...
changes: [decision-0094, decision-0095]  # THREE clauses across two records. TWO sit in a "What is not decided here" section and are no part of a ruling; the THIRD is inside decision-0095's ruling and is called out separately at the end of this comment, because an earlier draft said "neither any part of a ruling" while this record's own body measured a clause of decision-0095's D6 false -- a self-contradiction the corpus check caught. decision-0095: "The vendored branch where the overlay's OWN copy is present but unusable ... still keeps its token and still says nothing." decision-0094: the SECOND bullet of its own such list -- "keeps the token, names its own empty file, and says nothing" -- together with that paragraph's opening count, "Two shapes stay silent", which after decision-0095 closed the first bullet and this record the second is now ZERO. Each predecessor's own superseded_in_part_by carries its scope. NAMING 0094 HERE IS A CORRECTION CAUGHT IN REVIEW: an earlier draft named only 0095 and excused the omission on the ground that decision-0094:5 "reserved a question rather than asserting a claim". That is true of D5 and irrelevant to the bullet, which is a different clause in a different section and DOES assert a claim that this change falsifies -- and decision-0095 had already marked 0094 for bullet ONE of the very same list, on the very same reasoning, so leaving bullet two unmarked was an inconsistency rather than a judgement. decision-0093 records the repo paying for exactly this shape before: "The claim was wrong in two places and marked in neither." THE THIRD CLAUSE, and the one inside a ruling: decision-0095's D6 asserts "so Claude never delivers an invariants pointer into a vendored project and has nothing to report about", which this record's Consequences measure false -- and false WHEN WRITTEN, not falsified here, since the static import chain predates it. The rest of D6 stands and is relied on; decision-0095's own superseded_in_part_by carries the precise scope, and TRL-76 carries the fix. Declared as convention, not requirement -- decision-0045:143-144 is permissive ("MAY declare which artifact(s) it changes") -- and following the majority precedent decision-0094 measured and decision-0095 followed
...
- **`decision-0095` is corrected in two places: one disclosure, and — unlike its predecessors in
  this series — one clause inside its ruling.** The disclosure clause
