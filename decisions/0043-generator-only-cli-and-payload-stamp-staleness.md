...
superseded_in_part_by: [decision-0059, decision-0061, decision-0065, decision-0087]  # 2026-09-03 decision-0087 — ONE clause of rule 3: "no-op when either side is missing or empty". A stamp that is missing or empty now draws a marker rather than silence — TRELLIS_STALENESS_UNKNOWN when it is the installed plugin's reference/version (the session stays governed; only the drift check is withheld), TRELLIS_RULES_NOT_LOADED when it is the project's own .trellis/internal/version (a broken vendored overlay, which its two sibling files already refused loudly for), and on the LEGACY FLAT path .trellis/version — the path this rule literally names — the migration nudge now fires and says which stamp it could not read, with no TRELLIS_ marker because nothing there is ungoverned and the LAYOUT is the stale part with or without a stamp. Note that the compare itself moved from .trellis/version to .trellis/internal/version under decision-0051, which left no pointer here; every statement about "rule 3's stamp" since then, this one included, means the post-0051 path. Half of that departure predated 0087: a MISSING .trellis/internal/version had refused loudly for some time with no pointer, and 0087 marks the whole of it. What stands, unchanged: the rest of rule 3 — staleness is a file-to-file compare, warn on mismatch, no git/binary/network, legacy stamps draw the migration nudge — and every other rule in this record EXCEPT the parts decision-0059, decision-0061 and decision-0065 already reached (0065 narrowed rules 2-3 to vendored projects); those pointers sit in this same field. NOT covered by this clause at all, and therefore not a supersession: a stamp that EXISTS and cannot be opened (mode 000, a stale ACL), which rule 3 never addressed and 0087 closes as a gap (TRL-34)
