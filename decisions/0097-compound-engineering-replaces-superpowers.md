...
- **Ideation.** `ce-ideate`: *"Artifacts go under `<root>/ideation/`"* (`ce-ideate/SKILL.md:31`).
  No lifecycle instruction for them was checked.
...
marketplace. It also sets `superpowers@claude-plugins-official` to `false`, so a superpowers
install enabled at user scope stays off here. **That file reaches local checkouts, not cloud
sessions.** A cloud container never trusts the folder, so it never registers the marketplace or
installs project-enabled plugins (`plugins/trellis/README.md`, from `TRL-35`). There, the
environment's setup script has to install CE (`TRL-91`).
...
commit and the absence of any deletion instruction. Ideation is treated like plans on inference
alone: CE writes it beside plans, and its lifecycle was not checked.
...
- **What was about superpowers retires with it:** point 2's retention rule for superpowers' own
  specs, plans and SDD workspace, and point 3's rule that `.superpowers/` is git-ignored. This
  checkout's `.superpowers/` is deleted. The ignore line stays, as a legacy guard: a stale workspace
  in another checkout never shows up as untracked. The specs and plans already committed stay, under
  point 6.
...
- **The guard walks in `cli/` skip `.context/compound-engineering/`** at the root of the walk, CE's
  git-ignored scratch, as structurally outside the checkout. The match is that exact path, so a
  tracked `.context` directory anywhere else is still walked. A worker session writes scratch inside
  its own worktree, and walking it failed a local run on the agent's notes while CI, which never sees
  them, passed.
- **`CONCEPTS.md` is walked** once a CE skill creates it. A stale live claim there, such as one about
  ...
  in `README.md`.
- **`.gitignore`** ignores `.context/compound-engineering/`, and keeps `.superpowers/` as a legacy
  guard (point 7).
- **Cloud sessions lack CE until the environment installs it** (point 1). Until the Kodhama setup
  script does (`TRL-91`), a cloud session in this repository has none of the skills point 2 routes
  work through.
...
- **This record corrects its own drafts, on review.**
  - The first draft said CE gave no instruction to commit or delete a plan or learning. An
    independent Codex review found `ce-compound-refresh`'s commit and delete instructions. The search
    behind the claim had covered only `ce-plan`, `ce-brainstorm` and `ce-compound`, so the claim was
    never established. The quoted bullets in Context replace it.
  - A second `corpus-reviewer` pass found two internal contradictions. Point 7 retired `0085`'s point
    2 whole while keeping that point's correction of `0079`; it now separates the two. Point 4 gave a
    reason for exempting learnings that would also have exempted `CONCEPTS.md`; the exemption now
    rests on location, the line actually drawn.
  - Codex's review on the pull request found that point 1 claimed cloud sessions resolve CE from the
    settings file. This repository's own evidence (`TRL-35`) says they do not. It also found that
    removing superpowers' project entry left a user-scope install active. Point 1 now states both.
  - Claude's review on the pull request found that a rewrite had dropped the Context evidence for
    ideation. That bullet is restored.
