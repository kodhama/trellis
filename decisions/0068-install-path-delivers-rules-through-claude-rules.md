...
superseded_in_part_by: [decision-0070, decision-0088, decision-0091]  # decision-0091: 2026-09-04 -- ONE outcome, for ONE input, wherever D12 states it. Scoped by PARAGRAPH rather than by sentence, deliberately: two successive corpus reviews of decision-0091's branch each found a further sentence carrying the same proposition after the previous ones had been named, so enumerating sentences was itself the defect. (1) D12's WHOLE opening paragraph, :259-262 -- its topic sentence "install.sh refuses to render when the project already delivers the rules statically", its ground "there is no alternative: both static chains are loaded by the host before any hook runs", and its conclusion "rendering blindly ships guaranteed double delivery". (2) The read-inventory row at :291, which states the outcome for the CONTENT read of CLAUDE.md/AGENTS.md as `refuse`, in its AGENTS.md half only. (3) The closing what-this-does-not-cover clause at :318-319, "a repo carrying an inline block is *refused*, not repaired", in its "is refused" half only -- "not repaired" survives, this script still repairs nothing. Every one is false for exactly one input and no other: a managed block in an AGENTS.md that no CLAUDE.md imports with a standalone `@AGENTS.md` line is not read by Claude Code (decision-0057 rule 2), so the rendered .claude/rules/trellis.md is the only chain loaded, there is no second delivery to double, and that shape is now RENDERED OVER rather than refused. THIS RECORD CHANGES NONE OF THE FOLLOWING, which is a narrower claim than saying they stand -- some are already false for other reasons and are claimed by the decision-0088 segment below or by nobody: the same :291 row's CLAUDE.md half, which refuses exactly as before, with or without an import line; the read count and every other inventory row (decision-0091 D4 adds no read, and takes no position on whether D12's counts are still accurate -- :296 and :306-307 were overtaken by TRL-37, TRL-38 and TRL-44, and this pointer does not adopt them); D12's argument for WHY a content read exists at all -- the inline form is a fourth shape the existence tests miss -- which is untouched and is the reason the probe still runs on both files; and D12's "next simplification has to come here and argue itself", which decision-0090 D5 preserved deliberately and which decision-0091 discharges rather than excepts. | decision-0088: 2026-09-03 -- D5's CONSTANT-HEADER ruling (:154-156, "emits reference/trellis-b.md's posture prose as a shipped constant, not a choice") and every statement its reversal falsifies: the read-inventory summary at :296-298 ("exactly one reads a file's contents ... selects no posture"), the table row at :292 classifying .trellis/rules.toml as an EXISTENCE read selecting guidance text (it is now a CONTENT read of the strictness key), D9's sentence at :199 in full -- its "makes no posture choice" clause is reversed by this record; its "writes no rules.toml" clause was made false by decision-0070 D2's seeding, but decision-0070's pointer below names only :102, so this pointer records :199 rather than leave that half unclaimed -- and D12's closing "what still holds" at :312-313 ("no posture is selected"), which is now false and was the part that amendment turned on. The render now selects the header from .trellis/rules.toml's strictness, as the hook already did; D5's stated ground -- that AC2 forbids reading .trellis/ -- had already been removed by decision-0070 D2 and by D12's own amendment. STANDS: D5's footer sentence (the rows outrank the frozen sentence), D1, D11, the delivery mechanism, the payload edits and the stamp. | decision-0070: 2026-07-31 -- ONE clause: :102's "install.sh writes that file and nothing else new ... and never touches .trellis/". decision-0070 D2 has it seed .trellis/rules.toml. Everything else stands: the delivery mechanism, the Claude-only and project-scope-only bounds, and D12's read inventory, which gains a WRITE and no new read.
owner: agent
date: 2026-07-30
---
