...
# 0009 — How Bonsai improves itself (validation scenarios + feedback loop)

**Raised by:** the maintainer — *Bonsai's self-improvement loop improves the project or the
methodology, not Bonsai. So what improves Bonsai, beyond our design sessions?*

## Context

Bonsai's self-improvement loop (B6) lands on the project (adopt) or the methodology (adapt)
per the adherence axis (B8) — never on Bonsai-core. Bonsai is an improvement engine with no
input channel for its own improvement. Bonsai-core can only improve from **cross-instance
...
So "what improves Bonsai" has two honest answers — near-term validation, and an ongoing loop.
...
3. **Project with spec-kit installed** — strict-adopt / conductor. Tests Bonsai **enforcing
...
- **Signal source:** D1-surfaced friction — skips, overrides, gate-failures, divergences. The
  **gold signal is the human overriding Bonsai and being right** (a labeled correction).
- **Triage:** an agent (possibly an *independent* reviewer agent rather than Bonsai's own
...
- **Human gate (B3 + D2):** Bonsai **suggests, never merges**. Bonsai-deciding-to-change-Bonsai
...
- **Adherence interaction (`decision-0002`/B8):** adds a **third routing tier** — improvements
  land on the *project* (adopt), the *methodology* (adapt), or **Bonsai-core** (when the
  improvement is about *how processes are supervised* — an invariant/gate/dial/catalog — not a
  specific process). The Bonsai-core tier has the highest bar (cross-instance recurrence) or
  B7/minimal-first dies.

## Open questions

- The **critical-mass threshold**, and how to measure **instance diversity** honestly.
- Is the triage agent being **Bonsai itself** too circular even with the human gate? (Ties to
