...
# (TRL-37). That key is the one project file whose CONTENTS decide anything
# here; the managed-block marker is read too, but only to refuse. It NEVER runs
# a git
...
rendered rules file takes its posture header from that file's strictness. Either
way the posture is yours to change afterwards by editing .trellis/rules.toml; see
the "next steps" this script prints when it finishes.
...
f499b2e67de1e03a78b2280b568c54630def2240ba6c2a21508963a4bcb28f95  .claude-plugin/plugin.json
2e8fa90854879ee1edaba2eaa2bddb56677b49245263cadb4427f1c2f46f2900  .codex-plugin/plugin.json
3e969e059b4e979ee4f853680726adb3e7e80e8c34c2fa3b809203fc673a2284  README.md
a66780da23103beaf12432b418508966b31a2a3a787f9468a5b6cdaa8667c1ef  VERSION
289afe00d6159f846ef7c42fc61ae6cd9e754764cf8afdefecc61bceffc18f1a  hooks/codex-context.mjs
...
bf43e00dc9ea5cc0650043f0fb9a776617c9bf933dea74b29c74a6cff5065dec  hooks/staleness.sh
...
8ac8d42c5b32d4dff93a4e55f16d889c3bbd53de245f27ab71b9509552837708  reference/checksums
...
60ef4a91b92cb740405dd665348476c162f59f6e400645ad7ca02062bd8972ba  reference/version
...
if [ "$scope" = "project" ] && [ -n "$static_conflict" ]; then
  ...
  # spec-0005 AC2's second amendment permitted the first; the second is argued
  # in the test that pins the count (cli/install_script_test.go).
