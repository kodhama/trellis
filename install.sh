...
# plugin (kodhama/trellis#124, corrected design; supersedes the closed #128 attempt —
# see #128's own closing comment). This is NOT the retired end-user binary installer
# (kodhama-0007 rule 5, decision-0043 §4 — see the note appended there): it downloads
# no binary and, more importantly, makes exactly ONE decision (where to put the
# plugin) and composes NOTHING else. Every other decision — posture, which
# instructions file to patch, block style, hand-authored-content guarding — stays
# entirely inside plugins/trellis/skills/setup/SKILL.md, unmodified and identical
# whether the plugin arrived via marketplace, a pre-committed skills-dir vendor (this
# script), or the manual copy path. A second independent writer of that skill's
# *decision logic* is exactly the drift-risk class kodhama-0007 exists to close; this
# script is a mechanical copier of the plugin bundle only, same shape as the setup
# skill's own "copy, paste, verify" (kodhama-0007 rule 2) but one layer further out —
# it vends the *plugin*, not the *overlay* the plugin's skill later writes.
#
...
#
# WHAT THIS SCRIPT DOES, AND NOTHING MORE: fetches the whole plugins/trellis/ tree,
# verifies every byte against the manifest baked in below, and writes it to the
# resolved scope directory (overwriting the plugin's own prior files on a re-run —
# same idempotent-artifact principle as the rest of this family). It NEVER touches a
# project's .trellis/ — that is /trellis:setup's job entirely, not this script's —
# and it NEVER runs a git command that mutates anything (no add, no commit): it
# prints a suggested next command for project scope and leaves the commit to you.
#
...
# shasum or sha256sum. git only to resolve project scope's target directory — never
# required for personal scope. No binary, no network beyond the bundle fetch.
...
Flags:
  --scope personal|project   where to vendor the plugin. Also settable via
                              $TRELLIS_SCOPE (the flag wins if both are given).
                                ...
                                personal — ~/.claude/skills/trellis
                                           (every project on this machine; the
                                           default when not inside a git repo)
  --non-interactive           never prompt, even if a terminal is available
                              (automatic already when none is).
  --help                      this text.

Environment:
  TRELLIS_SCOPE           same as --scope; the flag takes precedence if both are set.
...
# so a bad --scope/env value fails instantly, before any network fetch.
requested=""
requested_origin=""
if [ -n "$SCOPE_FLAG" ]; then
  requested="$SCOPE_FLAG"; requested_origin="--scope"
elif [ -n "${TRELLIS_SCOPE:-}" ]; then
  requested="$TRELLIS_SCOPE"; requested_origin="\$TRELLIS_SCOPE"
fi
if [ -n "$requested" ]; then
  case "$requested" in
    personal|project) ;;
...
# --- The bundle manifest — baked in, covers the whole plugins/trellis/ tree ------
# Advance-guarded by cli/install_script_test.go:TestInstallScriptBundleManifestIsCurrent.
...
5a3ab72b69371df5529c911b8d961be6a5a2e8197f3ca948334f9516ed051248  reference/checksums
...
8aebf2efb0fea345c0cce7147aafb4459cc4b2b7b681fd579db2bee10063a8d2  reference/invariants.md
...
070635470599f6e57c376227eea9d1033d088987507584913580946837d56f84  reference/version
...
# --- 1. Fetch the bundle into a staging dir and verify it — nothing in the target -
#         directory is touched until every staged byte checks out against the
#         manifest above. This is the pin-then-verify-before-write shape (adapted
...
# --- 2. Scope — the one decision this script makes -------------------------------
...
if [ -n "$requested" ]; then
...
elif [ "$repo" -eq 1 ] && can_prompt; then
  {
  ...
  scope=personal
  scope_origin="default — not inside a git repository (git rev-parse --show-toplevel found none), so project scope is unavailable; installing personal scope instead"
fi
say "scope: $scope ($scope_origin)"

if [ "$scope" = "project" ] && [ "$repo" -eq 0 ]; then
  fail "project scope was requested ($scope_origin), but the current directory is not inside a git repository. Re-run from inside a git repo, or pass --scope personal (or TRELLIS_SCOPE=personal)."
fi
...
if [ "$scope" = "project" ]; then
  say ""
  say "Review the new files, then commit them yourself — this script never runs git:"
