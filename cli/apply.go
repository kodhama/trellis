...
//
// A rules file reached through a symbolic link is still classified, but both
// hooks show nothing read from it and only count its ignored entries (TRL-97 Q5):
// a committed link can point at any file the user can read. Item 2 and the
// closing report carry that to the agent, which reads the file itself here and
// would otherwise quote the ignored rows of whatever the link points at.
//
