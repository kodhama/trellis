...
#
#   FRAMEWORK=spec-kit TASK=eval/experiments/does-trellis-help/tasks/01-ambiguous-feature.md REPEATS=3 ./eval/experiments/does-trellis-help/run.sh
#
...
scaffold() {  # $1 = target dir. Non-interactive per research-0011.
  case "$FRAMEWORK" in
    ...
    spec-kit-lite)  # Spec Kit's rules as a plain AGENTS.md — no CLI, works with subagent workers
      mkdir -p "$1" && cp "$EXP/fixtures/spec-kit-lite.md" "$1/AGENTS.md" ;;
...
score() {  # $1 arm  $2 idx  $3 transcript  — one reviewer per rubric (extend to a panel via REPEATS)
  ...
  for rubric in invariants "${FRAMEWORK%-lite}"; do
    local card="$EXP/scorecards/$rubric.md"
    [ -f "$card" ] || continue
    local rp; rp="$(mktemp)"
...
run_arm() {  # $1 arm (baseline|trellis)  $2 idx
  ...
  local fix="$EXP/fixtures/$(basename "$TASK" .md)"
  [ -d "$fix" ] || fix="$EXP/fixtures/sample-app"
  cp -R "$fix"/. "$dir"/
