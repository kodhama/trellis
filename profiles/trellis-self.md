...
> and TRL-96 repairs 2026-09-14.** Read each verdict as of the first date. As of each repair date: every
...
>
> **A fifth staleness reached three rows and Delivery, and is repaired here too (2026-09-14).**
> TRL-96 made `cli-ci`'s `build-test` run on every PR and become a required status check on `main`, so
> the artifact-conformance check runs on every PR and a red one blocks the merge. Delivery,
> `inv-gate-at-handover`, `inv-independent-judgment` and `inv-bounded-context` said the check runs on
> every PR touching the corpus; the fourth note above keeps that wording as the state it recorded.
> `inv-gate-at-handover` also quoted `AGENTS.md` calling `build-test` advisory, and rested
> `default-on-but-skippable` on no gate blocking a merge. It is re-argued from the gate itself:
> independent review still does not block a merge, and the check is supporting evidence only
> (`decision-0098` point 7). No verdict, C1, C2 or confidence value changed.
...
- **delivery_relationship:** `supervisor` — the checks run *live* on this repo, not as an external
  ...
  artifact-contract check `cli-ci` runs on every PR
