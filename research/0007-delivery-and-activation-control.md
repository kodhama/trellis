...
- **Axis A — delivery *relationship*.** *Supervisor* (push): Trellis installed and live, constrains at
  runtime, stays current via the delivery channel, cleanly removable. *Consultant* (pull): Trellis not
...
| **Consultant** (pull/one-shot) | **minimal graft → reviewable PRs** (maintainer's working default); team-reviewed, no runtime, no self-regulation | **full graft** — bakes the whole apparatus in as a one-shot; host self-regulates *without* Trellis present |

Two cells have real backing already: **consultant × expressed-only** is the modality the maintainer
...
"consultant × +mechanism" really means *a consultant that installs a host-native supervisor and
...
## Independence in consultant mode (resolving the B3 concern)

The earlier worry — "consultant mode has no runtime, so independent verification (B3/invariant-5)
isn't automatic" — is **answered for application-time** and *partly* for ongoing:

- **Application-time (partly resolved — softened per Fable review):** consultant changes land as
...
- **Consultant × expressed-only** — lowest friction (referencing a repo sits *below* the compliance
...
per Fable review**): consultant × expressed-only is a real *wedge for reach*, but its "low friction"
...
checks *fit*, not *invariant-fidelity* (above); (c) consultant grafts **go stale**, so
"land-consultant-then-expand-to-supervisor" inherits drift — the upsell migration may be *harder*,
not cleaner, than starting as a supervisor. Net: pursue consultant mode for **reach and
...
- **The fork:** which cell ships first (intent-layer, owed to the maintainer). "Land-consultant,
...
- **Does consultant × expressed-only satisfy enough invariants to still be "Trellis,"** or is it
  Trellis-lite (#22) under a delivery name? (Ties the delivery fork to the partial-application question.)
- **Staleness protocol for consultant grafts** — how a pull-delivered result is re-synced when upstream
  Trellis evolves (the inverse of the supervisor's auto-currency).
- **Supply-chain / prompt-injection surface of consultant mode** (Fable review): an external repo
