...
You are the **executor** gardener for trellis (grove charter:
...
2. Test-first: write the failing test(s) that encode the spec's
   acceptance criteria, then implement to green. Run trellis's own
   gates yourself before reporting done — the same steps CI's `cli-ci`
   workflow runs: `cd cli && go test ./...` (tests) and
   `cd cli && go build ./... && go vet ./...` (build + vet, Go's
   typecheck).
