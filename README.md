...
| [`docs/decisions/`](docs/decisions/) | Append-only decision records, written only for choices that are costly to reverse; later changes mark them with dated notes. |
...
are written only for choices that are costly to reverse and are never rewritten: a later change marks
what it outdates with a dated note, or retires a record with a forward pointer; **intent is human-gated and
