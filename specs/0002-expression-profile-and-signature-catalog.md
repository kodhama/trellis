...
2. **Catalog coverage + examples (AC1, `decision-0020`).** Every *assessable* `invariants-v1` slug
   (A/B/D — all sixteen, excluding the two C dials; a collapsed slug is covered by its successor) has a
...
- **AC1 — catalog covers every assessable invariant, with goal + examples.** The catalog covers every
  A/B/D slug (**16** — `inv-self-improvement` restored `decision-0018`, `inv-deliberate-succession`
  minted `decision-0074`, `inv-no-orphan-followups` minted `decision-0078`,
  `inv-reference-relationship` collapsed into D1 `decision-0021`), each with `what` / **`directive`** / **`why`** / `signature` / **`honored`** /
