...
superseded_in_part_by: [decision-0058, decision-0071, decision-0076]  # 2026-08-01 decision-0071 — AC3/AC4s byte-identical managed block in CLAUDE.md, and the Codex bootstrap block in AGENTS.md, both retired for THIS repo: it self-applies through the plugin now, so CLAUDE.md is the @AGENTS.md adapter alone. The entrypoint-adapter principle stands; the managed-block SHAPE does not. A reader starting from this spec would otherwise implement a shape the repo now rejects.  # 2026-08-27 decision-0076 — the GROVE half of AC1 ("the full Layer-B method and Grove block exist in AGENTS.md"), and the two `.grove/` rows among §3's bounded current-truth surfaces: grove is retired and `.grove/` deleted, so AGENTS.md now carries no managed block at all. The Layer-B half of AC1 stands, as does the entrypoint-adapter principle; AC2's six routing statements stand with one reworded (it named `.grove/`). A reader starting from this spec would otherwise recreate a block no command can regenerate.
