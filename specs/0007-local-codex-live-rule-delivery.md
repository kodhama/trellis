...
- the sentinel-plus-fixed-footer boundary proof and activation-row duplicate
  check;
- the canonical inventory of every assessable slug, as names only;
...
slug names are permitted solely so the bootstrap can decide whether the
...
| Assembly and dedup | Exact single-placeholder expansion, source ordering/count, 8,000-byte guard, completion-sentinel placement, slug-inventory completeness, full/partial/no-context branches | S1, S3–S5, S19 |
