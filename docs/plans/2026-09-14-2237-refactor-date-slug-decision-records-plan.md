...
- **Execution profile:** one pull request against `main`, three implementation units (U1, U2 and U4) run inline, no plugin payload change.
...
- R5. *Dropped.* The maintainer took this requirement out on 2026-09-15, after his review question on PR #315, so the TODO check keeps the marker forms it has on `main`. AE4, KTD3 and U3, which served R5, went with it, as did the test added for R5 after Codex's review.
...
Product Contract unchanged, except that its three Deferred to Planning questions are answered and removed from Outstanding Questions (KTD4 and KTD5 answer two; the third left with R5), and that the maintainer dropped R5 on 2026-09-15.
...
- KTD8. **No new test.** The guard's deletion is proved by the Go suite still building and passing, and by a search showing that nothing outside records, plans and the ideation names the guard. The project is retiring guards (idea 4); #313's plan took the same stance.
...
- `shellcheck` is not installed locally, so `npm run quality` stops at `lint:shell` here. CI installs it and runs the gate.

### Sequencing

U1 and U2 are independent. U4 follows U2, because its notes name the paragraph U2 adds. All three land in one pull request.
...
- **Quality gate** (U1): `npm run quality` passes through `lint:js`; `lint:shell` needs `shellcheck`, which CI installs.
...
- R1 to R4 and R6 to R10 hold, and AE3 and AE5 hold on the branch. AE1 holds against TRL-97's record if that record merged first, and otherwise rests on the 2026-09-14 probe named in Dependencies. AE2 rests on git's conflict on a path two pull requests both add, and needs no check.
