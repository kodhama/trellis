...
- **Other walked-but-unfiltered paths.** About 18 other text files that `marketplaceCommands` reads are outside `cli-ci`'s filter: other workflows, `.github` templates, `core/schemas/`, `core/fixtures/`, the root brief, `.compound-engineering/`, `.devcontainer/`. The maintainer directed a separate Linear issue; `trellis-d1` holds it.
...
- **Dependencies:** U3.
- **Files:** `AGENTS.md`, `README.md`, `core/rubrics/artifact-contract.md`, `.claude/agents/corpus-reviewer.md`, `profiles/trellis-self.md`, `cli/docs_consistency_test.go`, `cli/surface_matrix_guard_test.go`, `.github/workflows/repo-hygiene.yml`
- **Approach:**
