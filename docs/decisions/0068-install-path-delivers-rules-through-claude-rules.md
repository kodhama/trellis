...
id: decision-0068
type: decision
status: approved  # maintainer intent act 2026-07-31, in session: "merge it", then "flip them to approved". Given after the record was shown naming D12's AC2 widening as the part that postdated the D1 ruling, so the act covers it explicitly. Drafted by agent. Three rulings are recorded, in order. (1) 2026-07-30, direction: "let's focus on claude only for the install path, potentially with the symlink alternative if you think it has merit". (2) 2026-07-30, ROUTING — the agent recommended `/trellis:setup` write the rendered file and said it would not proceed until ruled; the maintainer chose `install.sh`, which is D1, and this frontmatter records the ruling that reversed the agent's own recommendation. (3) 2026-07-30, wording — the maintainer accepted new install-path wording as a `decision-0053` exception; D5 below records that the exception proved UNNECESSARY on measurement, so the ruling is preserved but not exercised. (An earlier version of this line pointed at a "§Wording" section that does not exist.) §Open questions 1-3 carried those three acts and are now struck and folded into D1, D5 and D11. Questions 4 and 6 are also closed in this PR; **question 5 (do the vendored skills load at all) is the only one still open**, and it does not block implementation. (Two earlier versions of this line were wrong about which remain: the first said three, the second said "6 and below".) The measurements are the agent's; the ratification is not.
depends_on: [decision-0043, decision-0053, decision-0058, decision-0065, decision-0066]
superseded_in_part_by: [decision-0070, decision-0088, decision-0091]  # decision-0091: 2026-09-04 -- ONE outcome, for ONE input, wherever D12 states it. Scoped by PARAGRAPH rather than by sentence, deliberately: two successive corpus reviews of decision-0091's branch each found a further sentence carrying the same proposition after the previous ones had been named, so enumerating sentences was itself the defect. (1) D12's WHOLE opening paragraph, :261-264 -- its topic sentence "install.sh refuses to render when the project already delivers the rules statically", its ground "there is no alternative: both static chains are loaded by the host before any hook runs", and its conclusion "rendering blindly ships guaranteed double delivery". (2) The read-inventory row at :293, which states the outcome for the CONTENT read of CLAUDE.md/AGENTS.md as `refuse`, in its AGENTS.md half only. (3) The closing what-this-does-not-cover clause at :320-321, "a repo carrying an inline block is *refused*, not repaired", in its "is refused" half only -- "not repaired" survives, this script still repairs nothing. Every one is false for exactly one input and no other: a managed block in an AGENTS.md that no CLAUDE.md imports with a standalone `@AGENTS.md` line is not read by Claude Code (decision-0057 rule 2), so the rendered .claude/rules/trellis.md is the only chain loaded, there is no second delivery to double, and that shape is now RENDERED OVER rather than refused. THIS RECORD CHANGES NONE OF THE FOLLOWING, which is a narrower claim than saying they stand -- some are already false for other reasons and are claimed by the decision-0088 segment below or by nobody: the same :293 row's CLAUDE.md half, which refuses exactly as before, with or without an import line; the read count and every other inventory row (decision-0091 D4 adds no read, and takes no position on whether D12's counts are still accurate -- :298 and :308-309 were overtaken by TRL-37, TRL-38 and TRL-44, and this pointer does not adopt them); D12's argument for WHY a content read exists at all -- the inline form is a fourth shape the existence tests miss -- which is untouched and is the reason the probe still runs on both files; and D12's "next simplification has to come here and argue itself", which decision-0090 D5 preserved deliberately and which decision-0091 discharges rather than excepts. | decision-0088: 2026-09-03 -- D5's CONSTANT-HEADER ruling (:156-158, "emits reference/trellis-b.md's posture prose as a shipped constant, not a choice") and every statement its reversal falsifies: the read-inventory summary at :298-300 ("exactly one reads a file's contents ... selects no posture"), the table row at :294 classifying .trellis/rules.toml as an EXISTENCE read selecting guidance text (it is now a CONTENT read of the strictness key), D9's sentence at :201 in full -- its "makes no posture choice" clause is reversed by this record; its "writes no rules.toml" clause was made false by decision-0070 D2's seeding, but decision-0070's pointer below names only :104, so this pointer records :201 rather than leave that half unclaimed -- and D12's closing "what still holds" at :314-315 ("no posture is selected"), which is now false and was the part that amendment turned on. The render now selects the header from .trellis/rules.toml's strictness, as the hook already did; D5's stated ground -- that AC2 forbids reading .trellis/ -- had already been removed by decision-0070 D2 and by D12's own amendment. STANDS: D5's footer sentence (the rows outrank the frozen sentence), D1, D11, the delivery mechanism, the payload edits and the stamp. | decision-0070: 2026-07-31 -- ONE clause: :104's "install.sh writes that file and nothing else new ... and never touches .trellis/". decision-0070 D2 has it seed .trellis/rules.toml. Everything else stands: the delivery mechanism, the Claude-only and project-scope-only bounds, and D12's read inventory, which gains a WRITE and no new read.
owner: agent
date: 2026-07-30
---
...
`decision-0053:54-57` protects what the `annotation-vs-absence` experiment
...
than leaving it inferred.** `decision-0058:131` governs "any Claude hook
replacement" and requires that the old transport be removed or disabled in the
same change "so rules still arrive once"; `:197` sets the boundary — "never while
...
| 15 | `status: gated` earned (now `approved`) | **PASS** — self-check run after four independent reviews; three false rows corrected, one partial recorded, the blocking questions ruled. What remains open is **question 5 alone** — whether the vendored skills load at all — and it does not block implementation. The drift surface (q4) and VERSION (q6) were open when this row was first written and are now closed; the row is corrected rather than left to read as current |
