...
superseded_in_part_by: [decision-0098]  # decision-0098: 2026-09-14 — Decision point 6's STANDING RULE ONLY: "invoke the repo-owned `corpus-reviewer` before merging a change to `docs/decisions/`, `specs/`, `docs/research/` or `core/`", with that point's ground of "no CI check covering the contract", which was true when written. The sub-agent is retired; a Go test that cli-ci runs on every pull request touching the corpus applies artifact-contract conformance here, and the parts no program can decide are review guidance in AGENTS.md's conformance bullet. STANDS: points 1–5, and point 6's account of why the rule was declared rather than smuggled. NOT CLAIMED, and read as the history of what this change did: point 3's harvest bullet (the four corpus tokens "need no home" because the charter baked them in) and point 5's paragraph repairing .claude/agents/corpus-reviewer.md.
owner: agent
date: 2026-08-27
---
