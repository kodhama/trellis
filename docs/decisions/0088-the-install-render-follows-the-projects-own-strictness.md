...
superseded_in_part_by: [decision-0090]  # 2026-09-04 -- ONE clause, in ONE place: the Consequences bullet under "What is *not* claimed here" (:109-110), whose closing sentence reads "A third read still has to come and argue itself". That is the clause's only statement in this record. decision-0090 rules that the read-budget COUNT is stated and enforced by cli/install_script_test.go's exactly-N subtest and is changed by argument in the same commit, while a read that DECIDES -- selects, patches, writes, or removes a refusal -- still comes to the corpus first. Under that split the bullet is wrong only about WHERE, and only for a read that merely refuses: the third read (the top-level `governed` key, #267/74d9d25) only ever refuses and implements decision-0070 D5, already on main. The rest of that same bullet STANDS -- install.sh may not read .trellis/ generally or branch on other project state, and the widening is still "one key, for one purpose, counted and guarded". D5 IS NOT CORRECTED, and the distinction matters: its closing sentence is about the SECOND read, it attributes the demand to the TEST ("The test that pins the count said a second read 'has to come here and argue itself'"), and it counts that test as one of the two things that answered it -- under decision-0090 D1 that sentence is not merely untouched but supportive. An earlier draft of this pointer, and TRL-42 itself, both misplaced the corrected clause in D5; an independent corpus-reviewer pass caught it before merge. Also STANDS in full: D1's header selection from `strictness`, D2's fall-through to trellis-b, D3's recorded divergence on the unreadable file, and D4's copied parser and its pair-guard.
