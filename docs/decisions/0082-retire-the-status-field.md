...
> **Dated note, 2026-09-14 — TRL-99 (PR #315):** Point 3's "Supersession is marked by the forward pointer" and the TRL-98 note's "full retirement still uses `superseded_by`" no longer hold for a record retired in full with no successor record, which gets a dated note instead of `superseded_by`. `AGENTS.md`'s *Operating method* states the current rule.
...
- **Empirically the working state was never used.** Before `decision-0081`, **not one** of the 73
  ...
  2026-08-28. At landing, and scoped the same way — `docs/decisions/` + `docs/research/` — the corpus is 80
...
trailing comment recording a specific maintainer intent act** (`docs/decisions/0046`: *"the first
decision ratified under its own rule"*; `docs/research/0012`: *"ok to merge!"*). Those comments are the
