...
superseded_in_part_by: [decision-0077, decision-0083]  # 2026-08-30 decision-0083 — D4's "Writes stay agent-mediated and human-consented" clause, AS IT APPLIES TO ROW REPAIR ONLY: consent for a reconciliation write to .trellis/rules.toml is the adoption act plus the session announcement, not a per-write confirmation, because quarantine loses no value (an unknown or duplicate row is commented out with dated provenance, never deleted, and every prior value survives verbatim). A reader who is not told this would believe per-write consent still governs row repair. What stands: "the hook never writes" (untouched, and still pinned by cli/plugin_hook_test.go), the announcement, the decline bullet, and D4's disposition of silence as already amended by decision-0077 below. D1-D3 and D5-D7 untouched. If a repair class ever becomes destructive again, the clause re-engages in full.  # 2026-08-28 decision-0077 — D4's silence disposition only: the "accept, or no objection → seed … governed at 14/14 from the next turn" bullet and the "silence never reads as refusal" rationale, which the hook never implemented (it names decline and explicit-accept, and says "the project is never governed" without the file). What stands: the announcement, that it injects no rules on that turn, the decline bullet, and "the hook never writes". D1-D3 and D5-D7 untouched
owner: agent
date: 2026-07-31
---
...
> **Dated note, 2026-09-15 — TRL-90 (PR #318):** D6's statement that the hook requires
> `CLAUDE_PLUGIN_ROOT` no longer holds for Droid's exact
> `/PLUGIN_ROOT_NOT_EXPANDED_ERROR` sentinel: the hook derives the effective plugin root from its
> own installed location for that value only. `decision-2026-09-15-droid-plugin-root-sentinel-self-locates`
