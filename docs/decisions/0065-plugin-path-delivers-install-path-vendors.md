...
superseded_in_part_by: [decision-0068, decision-0070, decision-0072, decision-0093]  # 2026-09-06 decision-0093 — ONE clause of :191-193, the "Rules arrive exactly once" paragraph: "A project that still has an overlay keeps its import transport and the hooks inject nothing". MEASURED false on Codex — staleness.sh emits zero bytes on a vendored project, but codex-context.mjs reads the overlay's three files and injects them, which is why 0093 could rule on the pointer inside that injected prose at all. The DISCRIMINATOR half of the same paragraph stands (both hooks do discriminate on .trellis/internal/), and so does "a project without one has no import block and receives the injection". Note the neighbouring :194-195 sentence was already re-pointed here by decision-0068 while this one was not, and plugins/trellis/README.md carried the identical "on both hosts" symmetry until 0093 corrected it — the claim was wrong in two places and marked in neither.  # 2026-08-02 decision-0072 retired /trellis:setup; every clause of this record that constrains that skill is now VACUOUS rather than false — there is no skill left to constrain. The reasoning stands as history and the record is not edited. Specifically: "setup writes exactly one file ... and nothing else, ever" is vacuous, not wrong.  # 2026-07-31 decision-0070, TWO clauses: the PLUGIN/INSTALL SPLIT at :26-29 -- "install.sh is vendoring and never configures ... and continues to never touch .trellis/" (0070 D2 has install.sh seed .trellis/rules.toml; one config file, in the repo the user pointed the installer at, and the no-silent-overlay purpose is untouched). NOTE: an earlier pointer named ":18-19's setup writes exactly one file ... ever" instead. That was the wrong clause -- it binds the SKILL, and D2 does not change what /trellis:setup writes, and :113-114's ".trellis/rules.toml is the opt-in signal ... never governed by surprise" (0070 D4 lets a user-scoped install govern an unadopted project, but only after announcing it in that project -- "never governed" is released, "by surprise" is preserved, and the announcement is what preserves it). 2026-07-30 decision-0068 — TWO clauses only: :143-146's "both hooks take the same two paths and use the same discriminator" (the Claude hook gains a third path plus a coexistence branch; the Codex hook keeps two, so the symmetry this clause states is deliberately broken), and :194-195's "There is no state in which both paths carry the payload — that is tested, not asserted" (a project can hold a vendored overlay AND a rendered file at once; the coexistence branch exists because that sentence became false). Everything else in this record stands — setup's one file and the no-vendoring direction are untouched. (This sentence used to claim the plugin/install split was untouched too; decision-0070 D2 is exactly what breaches it, so the claim was false in the same edit that preserved it.)
owner: agent
date: 2026-07-27
---
